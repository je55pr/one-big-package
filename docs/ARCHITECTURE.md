...
Ordinary Ratchet movement in R&C1, GC and UYA worlds delegates planar/jump/crouch integration to `OBP.RAC1.Player.Rac1RatchetMovementController`, which consumes neutral `PlayerControlIntent` / `PlayerContactFacts` and emits native-world displacement per 60 Hz update. Reusing this R&C1 retail-derived controller across the trilogy is an explicit OBP design choice, not evidence that GC or UYA shared the same native controller. Godot converts the result to engine velocity and keeps ownership of camera-relative intent construction, reconstructed collision, slopes and floor/ceiling contact. Development fly/noclip and manual respawn remain separate host features; the former DebugPlayer speed/jump/gravity calibration is retired from ordinary play.
