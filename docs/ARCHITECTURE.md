...
Animation selection is source-owned rather than Godot-owned. Providers with recovered selectors implement `IPlayerAnimationControllerProvider`; R&C1 supplies `Rac1PlayerAnimationPresentationController`, which owns the retained 3→4 locomotion entry, 7/8 launch-context jump choice, direct landing return and sequence-23 wrench one-shot rules and emits the corresponding native sequence/action-state keys. `PlayerAvatarView` implements only `IPlayerAnimationPresentationSink`: it validates the resolved clip id and renders frames from the supplied clock origin/loop policy, with no source-game transition table. Composition uses `GenericPlayerAnimationPresentationController` only when a provider explicitly lacks a native controller. GC/UYA currently have no native player-avatar provider, so those worlds retain the debug capsule rather than borrowing the R&C1 Ratchet avatar. Physics and movement remain authoritative outside this presentation mapping.
