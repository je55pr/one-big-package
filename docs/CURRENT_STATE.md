...
The shared codec now also preserves a separately retail-validated animation binding stream: persistent VU0 matrix-slot state, current-record skin-control bits, raw skin-local positions, parent-relative `common_trans` translations and sequence bounding spheres. `GcUyaMobyPose` evaluates multi-joint frames in `OBP.PS2`. As a deliberately OBP-created showcase admission, Veldin oClass 6800 / instance 665 / sequence 2 is exposed through `RuntimeAnimatedMesh`: 33 joints, 9 frames at 7.5 fps and 2,390 rendered triangles across two surfaces. Every promoted frame stays inside the sequence's authored retail sphere. The original 221,349-triangle Veldin Moby census is preserved as 218,959 dynamic + 2,390 animated triangles. This does **not** claim recovered native animation-state selection. See [`../research/UYA_MOBY_ANIMATION.md`](../research/UYA_MOBY_ANIMATION.md).
