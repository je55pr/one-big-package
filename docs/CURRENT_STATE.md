...
R&C1 campaign travel now also has a player-facing Godot galactic-map presentation: it renders only the native `Rac1PlanetMapSnapshot` (`CurrentLevel` plus recovered `GalacticMap` admission order), and a selection enters the existing two-phase `Rac1CampaignRuntimeSession` travel handoff. Godot does not infer unlocks from the provider catalogue. The current map styling and `M` / controller Start shortcut are explicitly OBP host presentation choices rather than a pixel-perfect retail UI or recovered binding. The recovered opening state consequently shows no invented travel targets until native campaign state admits destinations. The four recovered campaign blocks now persist through a versioned OBP host file and are restored on a fresh process. A retail-backed two-process smoke proves the bounded route `0 → 1 → 2 → 1`, reload at level 1 with admission order `[1,2]`, then `1 → 2 → 1` again. The smoke injects only proven progression-dispatch values `0x25` and `0x26`; ordinary gameplay still has no evidence-backed mission/objective producer for those events, so it does not fabricate unlock triggers, completion or checkpoints.

R&C1 ordinary player movement now also leaves the provisional debug-physics path. `Rac1RatchetMovementController` reproduces the authority 60 Hz ground acceleration/stop recurrence, variable-height jump anticipation/hold, distinct rise/fall gravity, air control, ledge falling, recovered crouch slide-down and state-specific facing recurrence. Godot remains collision authority. Controlled retail witnesses also pin respawn reset-to-player-start, the normal-facing yaw envelope, the stick-to-control-heading target rule `G+0x100 = WrapPi(controlHeading - stickAngle)`, and the raw left-stick law: 48-count per-axis trim, 76-count saturation, exact radial activation at remapped magnitude 19, the observed 62/63 walk/run bracket and no diagonal speed boost. That shaping is now applied in engine-independent `OBP.RAC1` before an engine-neutral control basis maps direction into the host plane; full-stick movement witnesses remain unchanged. Ordinary R&C1 play also uses the recovered type-0 camera producer, including manual horizontal recurrence, the recovered vertical secondary dead zone/orbit geometry, vertical follow, radial follow and the bounded obstruction response. Special camera modes/profile-selection producers remain outside the current boundary. The exact translational walk/run selector remains unresolved. See [`../research/RAC1_PLAYER_MOVEMENT.md`](../research/RAC1_PLAYER_MOVEMENT.md) and [`../research/RAC1_CAMERA_ARCHAEOLOGY.md`](../research/RAC1_CAMERA_ARCHAEOLOGY.md).
...
- `tools/rac1-veldin-play-smoke.ps1` is the end-to-end R&C1 playability gate. From a clean ephemeral `rac1:LEVEL0` start it uses only ordinary input to prove the six-round Bomb Glove opening grant is usable, horizontal and recovered vertical type-0 camera behavior is visible, the supported class-749 witness moves and naturally damages Ratchet, both hostile and class-500 Wrench contact are practically reachable, and the Veldin death plane reaches the recovered death boundary then automatically restarts at authored class 0 with four Nanotech, reset motion/yaw, synchronized player/camera presentation and resumed control. Natural-death acceptance does not press the development R-respawn key. It does not teleport actors, inject success state or call consequence handlers. `tools/rac1-combat-contract-smoke.ps1` is separately named synthetic coverage and must not be cited as ordinary-play proof; see [`RAC1_VELDIN_SMOKES.md`](RAC1_VELDIN_SMOKES.md);
...
- broader Ratchet controller/presentation fidelity beyond the recovered R&C1 run/jump/fall/crouch/facing/raw-stick/type-0-camera slice, especially the exact translational walk/run selector, unrecovered special camera modes/profile-selection producers and special traversal;
