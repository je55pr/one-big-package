...
public static class UyaLevelSettings
{
    ...
    public sealed record Settings((double R, double G, double B)? BackgroundColour, (double R, double G, double B)? FogColour,
        ...
        float DeathHeight, bool IsSphericalWorld, (float X, float Y, float Z) SphereCentre,
        (float X, float Y, float Z) ShipPosition, float ShipRotationZ, int BlockOffset);
    ...
    public static Settings Parse(byte[] gameplay)
    {
        ...
        float[] finite = [F(0x18), F(0x1c), F(0x20), F(0x24), F(0x28), F(0x30), F(0x34), F(0x38), F(0x3c), F(0x40), F(0x44), F(0x48)];
        ...
        return new Settings(Rgb(0x00), Rgb(0x0c), F(0x18), F(0x1c), F(0x20), F(0x24), F(0x28), S(0x2c) != 0,
            (F(0x30), F(0x34), F(0x38)), (F(0x3c), F(0x40), F(0x44)), F(0x48), block);
    }
}
