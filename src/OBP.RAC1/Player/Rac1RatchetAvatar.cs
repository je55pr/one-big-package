...
public static class Rac1RatchetAvatar
{
    ...
    public sealed record Asset(
        ...
        IReadOnlyList<double[]> StandingFrames,
    ...
    public static Asset Decode(Rac1LevelCore.Core core)
    {
        ...
        var standing = sequences[StandingSequenceId].Value
        ...
        if (standing.Frames.Count != 10 ||
            ...
            standing.Frames.Any(frame =>
                !Rac1MobyPose.CanPoseRatchetHierarchy(cls.Mesh, cls.Joints, frame)))
        {
            throw new InvalidDataException(
                "R&C1 Ratchet sequence 0 no longer matches the pinned standing loop.");
        }

        var frames = standing.Frames
        ...
        var standingBounds = Measure(frames.SelectMany(frame => frame));
        float fps = standing.ConstantTransitionRate * NtscUpdateHz;

        // Native RAC1 Moby space is Z-up. Placement yaw is the
        // native Z rotation, with the unrotated heading carried on +Y; adapters
        // may remap axes later, but this asset deliberately does not.
        return new Asset(
            cls.Mesh,
            surfaces,
            frames,
