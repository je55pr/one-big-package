...
2. **Broader multi-joint animation/state semantics** — the shared VU0 binding
   ...
   combat/get-hit/death and the targeting-state boundary. State-2 walk is
   pinned to sequence 3 for the state's lifetime, targeting preserves its
   weapon-context sequence rather than selecting fixed strafe clips, and a
   deterministic retail witness plus executable handoff pins ordinary Cross
   jump to sequence 7/state 7. See
