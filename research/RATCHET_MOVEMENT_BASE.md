...
1. **Input promotion and control-heading policy.** Promote the code-backed raw-byte trim/clamp and exact 0.25 activation law without inventing extra interpolation; preserve the live 62/63 walk/run displacement bracket until its translational selector is traced. The ordinary target consumer field is now identified as `0x00166dd8`; recover the retail camera/follow/recenter policy that produces it. DebugPlayer's presentation-camera heading remains host policy rather than a recovered retail chase-camera law.
