...
This follow-up is dispositioned as a **host normalized-input / outer-range discrepancy**, not a reason to change the recovered 48-count trim, 76-count scale, activation radius or native movement constants. The exact translational selector also remains unresolved inside the retained retail 62/63 bracket, so lowering `FirstObservedRunMagnitude` to accommodate one host device would incorrectly convert a host calibration symptom into a retail claim. The next bounded implementation task should capture raw SDL joy-axis and InputMap action strength together at cardinal extrema, then harden the host boundary so physical full-scale directions map consistently into the retail-derived normalized input contract without changing `Rac1AnalogueInput` evidence constants.
