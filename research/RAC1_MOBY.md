...
`Rac1WorldImport` preserves every parsed Moby placement as an individual neutral `RuntimeDynamicObject`. Renderable class geometry stays model-local, is grouped by the already-recovered class texture linkage, and is placed by the retail-pinned R&C1 Moby transform. Only Ratchet's separately selector-backed player preview remains on `RuntimeAnimatedMesh`; ordinary classes stay on their rest geometry unless source-game evidence admits a runtime object-animation role.
