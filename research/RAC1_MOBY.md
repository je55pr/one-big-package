...
`Rac1WorldImport` now preserves every parsed Moby placement as an individual neutral `RuntimeDynamicObject`. Renderable class geometry stays model-local, is grouped by the already-recovered class texture linkage, and is placed by the retail-pinned R&C1 Moby transform. Proven animated specimens remain on the existing `RuntimeAnimatedMesh` path rather than being double-rendered, while classes with no recovered payload/high-LOD geometry remain explicitly meshless.

Across all 19 authority levels this accounts for all **16,232** authored placements: **15,246** have linked dynamic render geometry, **113** hand off to the existing animated path, and **873** remain no-geometry objects. The 873 no-geometry placements exactly match the previously recovered population whose class payload is unavailable to this geometry path. Linked dynamic objects contribute **9,622,327** triangles; no retail geometry is guessed for the remainder. Per-level counts are frozen in `research/generated/rac1-dynamic-moby-runtime-census.json`.
